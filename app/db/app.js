...
var log = require('debug')('smooth:db:log');
...
db.open(function(err, db) {
  ...
  if (config.mongodb.admin === true) {
    ...
    async.forEachSeries(config.mongodb.auth, function(auth, callback) {
      log("Connecting to " + auth.database + "...");
      ...
      if (typeof auth.username != "undefined" && auth.username.length != 0) {
        connections[auth.database].authenticate(auth.username, auth.password, function(err, success) {
          ...
          if (!success) {
            errlog('Could not authenticate to database "' + auth.database + '"');
          }
...
if (require.main === module){
  ...
  log("Mongo Express server listening on port " + (config.site.port || 80));
}else{
  //as a module
  log('Mongo Express module ready ! ');
